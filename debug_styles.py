...
def test_export_styles():
    ...
    response = requests.get('https://lemaitremot-edu.preview.emergentagent.com/api/export/styles')
    ...
    response = requests.get('https://lemaitremot-edu.preview.emergentagent.com/api/export/styles', headers=headers)
    print(f"Status: {response.status_code}")
    data = response.json()
    print(f"Styles count: {len(data.get('styles', {}))}")
    print(f"User is pro: {data.get('user_is_pro', False)}")
    ...
    doc_response = requests.post('https://lemaitremot-edu.preview.emergentagent.com/api/generate', json=doc_data)
    if doc_response.status_code == 200:
        ...
        export_response = requests.post('https://lemaitremot-edu.preview.emergentagent.com/api/export', json=export_data)
